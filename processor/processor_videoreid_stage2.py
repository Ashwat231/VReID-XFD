...
def do_train_stage2(
    ...
    if left != 0:
        i_ter += 1
    text_features = []
    print("Collecting all text features ...")
    ...
    print("Text features collected. Begin training!")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, batch_data in enumerate(train_loader_stage2):
            ...
            # ====== Metadata (for PBP prompts) ======
            meta_for_gate = None
            if altitude is not None and cfg.MODEL.PBP_META:
                altitude = altitude.to(device)
                distance = distance.to(device)
                angle = angle.to(device)
                meta_for_gate = {'altitude': altitude, 'distance': distance, 'angle': angle}
    ...
    logger.info("Stage 2 total running time: {}".format(total_time))
