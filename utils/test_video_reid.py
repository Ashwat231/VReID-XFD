...
def extract_feat_sampled_frames(model, vids, cam, use_gpu=True):
    ...
    vids_flip = torch.flip(vids, dims=[4])  # flip width (W)
...
def _feats_of_loader(model, loader, feat_func=extract_feat_sampled_frames, use_gpu=True):
    ...
    for batch_idx, batch_data in enumerate(loader):
        ...
        # Support both 3-tuple and extended 8-tuple batches
        if isinstance(batch_data, (list, tuple)):
            if len(batch_data) >= 3:
                vids, pids, camids = batch_data[0], batch_data[1], batch_data[2]
            else:
                vids, pids, camids = batch_data
        else:
            # Fallback: assume dataloader yields exactly three items
            vids, pids, camids = batch_data

        f = feat_func(model, vids, camids, use_gpu=use_gpu)
        qf.append(f if f.dim() == 2 else f.reshape(f.size(0), -1))
